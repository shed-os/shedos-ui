...
//! with alpha blending against the existing pixels (the wallpaper).
...
/// portable to a developer's hand-installed font as well.
...
impl FontFace {
    ...
    /// `color` is sRGB (R, G, B); `alpha` is the overall opacity (0..=255)
    ...
    pub fn render(
        ...
        for ch in text.chars() {
            ...
            // Top-left of the glyph bitmap, given fontdue's bottom-up
            // (xmin, ymin) where ymin is from baseline going down.
